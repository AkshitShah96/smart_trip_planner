...
class ChatNotifier extends StateNotifier<ChatState> {
  ...
  void _handleStreamChunk(StreamChunk chunk) {
  ...
  Future<void> sendMessage(String content) async {
    ...
    try {
      ...
      final response = await _streamingService.generateStreamingResponse(
        userInput: content,
        chatHistory: state.messages.map((m) => m.toChatMessage()).toList(),
        lastItinerary: state.lastItinerary,
        isRefinement: _isRefinementRequest(content),
      );

      // Update the message with final response
      final updatedMessage = aiMessage.updateWithStream(response);
      state = state.updateMessage(aiMessage.id, updatedMessage);

      // Update last itinerary if response contains one
      if (response.itinerary != null) {
        state = state.copyWith(lastItinerary: response.itinerary);
      }
